...
- `CATEGORY_LABELS_EN` / `CATEGORY_LABELS_RU` / `CATEGORY_LABELS_UK` /
  `CATEGORY_LABELS_BE` — localised category headers
...
Each firmware entry carries copy in four languages:

- `excerpt*`, `summary*`, `history*`, `highlights*` — `En`, `Ru`, `Uk`, `Be`
- `En` and `Ru` are **required**; `Uk` and `Be` are optional but should be kept
  populated for every public firmware. Consumers are expected to fall back from
  `Uk`/`Be` to `Ru` (or `En`) when a translation is missing.

## Editing rules

1. Edit `data.ts` directly. Keep the EN, RU, UK and BE fields in sync.
